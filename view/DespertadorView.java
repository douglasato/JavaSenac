...
public class DespertadorView {
        ...
        while (sair == false) {
            ...
            if (horaAtual >= horaDespertar) {
                if (minutoAtual >= minutoDespertar) {
                    if(segundoAtual >= segundoDespertar){
                        if (podeAdiar()) {
    ...
    public static void configurarSegundo(){
        System.out.println("Digite abaixo o(s) segundo(s) que deseha configurar o despertador e tecle Enter: ");
        segundoDespertar = scnInput.nextInt();
        if(segundoDespertar == segundoAtual){
            if (segundoDespertar < segundoAtual) {
